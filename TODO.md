...
- [ ] **Pulizia `slot_overrides` passati — puramente estetica, NON urgente** (annotato 2026-08-21): le righe "al completo" (blocco prenotazioni online per giorno/servizio: `is_closed=0, max_covers=0, slot_time NOT NULL`) di date **già passate** restano nel DB per sempre (non cancellate; spariscono solo dall'elenco "Disponibilità online", filtrato a `override_date >= CURDATE()`). Sono **innocue** (effetto solo sulla data passata) e in numero irrisorio → nessun impatto reale su spazio/performance. Se un domani si vuole ripulire per igiene DB: cron banale tipo `DELETE FROM slot_overrides WHERE override_date < CURDATE() AND is_closed=0 AND max_covers=0`. **Attenzione a NON allargarlo** alle chiusure/emergenze passate (`is_closed=1`) o agli override storici senza valutare se servono per storico/statistiche. Stesso ragionamento dei clienti "solo nome" (righe minuscole, crescita trascurabile).
